...
var require_tool_cache = __commonJS((exports) => {
  var __dirname = "/Users/pftg/dev/_oss/actions-hugo/node_modules/@actions/tool-cache/lib";
  var __createBinding = exports && exports.__createBinding || (Object.create ? function(o, m, k, k2) {
    if (k2 === undefined)
      k2 = k;
    Object.defineProperty(o, k2, { enumerable: true, get: function() {
...
// src/get-url.ts
function getURL2(os, arch, extended, version) {
  const extendedStr = (extended2) => {
    if (extended2 === "true") {
      return "extended_";
    } else {
      return "";
  ...
  const ext = (os2, version2) => {
    if (os2 === "windows") {
      return "zip";
    } else if (usePkg(os2, version2)) {
      return "pkg";
    } else {
      return "tar.gz";
    }
...
async function createWorkDir() {
  const workDir = path.join(getHomeDir(), "actions_hugo" /* WorkDirName */);
  await io.mkdirP(workDir);
  core.debug(`workDir: ${workDir}`);
  return workDir;
}
async function createTempDir(workDir) {
  const tempDir = path.join(workDir, "_temp" /* TempDirName */);
...
async function installer(version) {
  ...
  const archName = getArch(process.arch, osName);
  core.debug(`Processor Architecture: ${archName}`);
  const toolURL = getURL2(osName, archName, extended, version);
...
async function showVersion(cmd, args) {
  ...
  result.exitcode = await exec3.exec(cmd, args, options);
  core2.debug(`command: ${cmd} ${args}`);
  core2.debug(`exit code: ${result.exitcode}`);
  core2.debug(`stdout: ${result.output}`);
  return result;
}
async function run() {
  const toolVersion = core2.getInput("hugo-version");
  let installVersion = "";
  let result = {
    exitcode: 0,
    output: ""
  ...
  if (toolVersion === "" || toolVersion === "latest") {
    installVersion = await getLatestVersion("gohugoio" /* Org */, "hugo" /* Repo */, "brew");
  } else {
    installVersion = toolVersion;
  }
  core2.info(`${"Hugo" /* Name */} version: ${installVersion}`);
  await installer(installVersion);
  result = await showVersion("hugo" /* CmdName */, ["version" /* CmdOptVersion */]);
  return result;
}
...
(async () => {
  try {
    ...
    core3.setFailed(`Action failed with error ${e.message}`);
  }
})();
